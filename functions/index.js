...
exports.getMALAnimeRecommendations = onCall(
    async (request) => {
        ...
        const supplied_seeds =
            Array.isArray(request.data?.seeds) ?
                request.data.seeds :
                Array.isArray(request.data?.seed_ids) ?
                    request.data.seed_ids.map((mal_id) => ({
                        mal_id,
                    })) :
                    [];

        const seeds = supplied_seeds
            .filter((item) =>
                item &&
                (
