...
exports.requestMediaOnSeerr = onCall(
    ...
    async (request) => {
        ...
        const {base_url, api_key: encrypted_api_key} = connection.data();
        const api_key = decrypt_seerr_api_key(
            encrypted_api_key, seerr_credential_encryption_key.value()
        );
        ...
        try {
            ...
            // Seerr returns 409 when this title has already been
            // requested — treat that as success from the user's side.
            if (error.status === 409) {
