...
notification_button.setAttribute("aria-label","Open notifications");
...
notification_panel.innerHTML='<div class="notification-head"><div><strong>Notifications</strong><span>New episodes and chapters</span></div><button type="button" class="notification-read-all">Mark all read</button></div><div class="notification-list"><p class="notification-empty">Loading notifications…</p></div>';
...
const notification_read_all=notification_panel.querySelector(".notification-read-all");
...
function render_notifications(){
    const unread=notification_items.filter(item=>!item.read).length;
    notification_badge.hidden=unread===0;
    notification_badge.textContent=unread>99?"99+":String(unread);
    notification_read_all.hidden=unread===0;
    ...
    if(!notification_items.length){
        ...
        empty.textContent="No release notifications yet.";
    ...
    notification_items.forEach(item=>{
        ...
        title.textContent=item.title||"New release";
...
notification_button.addEventListener("click",event=>{
    ...
    if(will_open)load_notifications();
});

notification_list.addEventListener("click",async event=>{
    ...
    try{
        if(!item.read){
            const mark_read=httpsCallable(functions,"markEntertainmentNotificationRead");
            await mark_read({notification_id:item.id});
            item.read=true;
            render_notifications();
        }
    }catch(error){
...
notification_read_all.addEventListener("click",async()=>{
    try{
