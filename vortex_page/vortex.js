...
async function startLobby(user) {
  //^ URL of the protected Vortex camera
  ...
  //^ Create an image element that will display the camera stream
  const vortex_streamImg = document.createElement("img");

  //^ Stop function if camera container is not found
  if (!vortex_cam_container) {
    console.error("Missing cam containers in HTML");
    return;
  }

  //^ Request a firebase authentication token for the currently logged in user.
  const token = await user.getIdToken(true);

  //^ Add the authentication token to the camera stream URL. The server should verify this token before allowing access.
  vortex_streamImg.src = `${vortex_stream}?token=${encodeURIComponent(token)}`;


  //^ Firebase tokens eventually expire. Every 50 minutes, request a new token and reconnect the camera stream using the refreshed token.
  setInterval(async () => {
    //^ Use the latest authenticated Firebase user if available.
    const u = auth.currentUser || user;
    //^ Stop if no authenticated user exists.
    if (!u) return;
    //^ Request a refreshed authentication token.
    const t = await u.getIdToken(true);
    //^ Update the camera stream URL with the new token. The timestamp prevents the browser from reusing a cached version of the previous stream URL.
    vortex_streamImg.src = `${vortex_stream}?token=${encodeURIComponent(t)}&ts=${Date.now()}`;
  }, 50 * 60 * 1000);

  //^ Remove any previous content from the camera container.
  vortex_cam_container.innerHTML = "";

  //^ Add the camera stream image to the page.
